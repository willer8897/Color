...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if(self)
    {
        self.manager = [NCBoardManager sharedManager];
        appDelegate = [[UIApplication sharedApplication] delegate];
        NSNotificationCenter *center = [NSNotificationCenter defaultCenter];
        [center addObserver:self selector:@selector(boardConnected:) name:NCBoardDidConnectNotification object:nil];
        [center addObserver:self selector:@selector(boardDisconnected:) name:NCBoardDidDisconnectNotification object:nil];
    }
    return self;
}
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    [self.outputButton1 setTitle:@"Output 0" forState:UIControlStateNormal];
    [self.outputButton2 setTitle:@"Output 1" forState:UIControlStateNormal];
    [self.outputButton3 setTitle:@"Output 2" forState:UIControlStateNormal];
    [self.outputButton4 setTitle:@"Output 3" forState:UIControlStateNormal];
    [self.outputButton5 setTitle:@"Output 4" forState:UIControlStateNormal];
    [self.outputButton6 setTitle:@"Output 5" forState:UIControlStateNormal];
    [self.outputButton7 setTitle:@"Output 6" forState:UIControlStateNormal];
    [self.outputButton8 setTitle:@"Output 7" forState:UIControlStateNormal];
}
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    ...
    if (captureImage)
    {
        ...
        NSString *text = (self.rgbLabel.text);
        ...
        CGRect rect = CGRectMake(50,0,image.size.width, image.size.height);
    ...
    for (int i = 0; i < [appDelegate.targets count]; ++i)
    {
        ...
        if(!t.on)
        {
            ...
            if(connected)
            {
                ...
                if(check >= 1)
                {
                    NSLog(@"turning off output");
                    output = (output ^ new);
                    NSLog(@"Output: %c", output);
                    NSError *error = nil;
                    if(![self.manager setDigitalOutput:output error:&error])
                    {
...
{
    connected = TRUE;
    //set all outputs to false since the board mgiht have been disconnected in any state
    uint8_t output = 0;
    NSError *error;
    if(![self.manager setDigitalOutput:output error:&error])
    {
    ...
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Board Connected" message:@"The controller board has been connected" delegate:self cancelButtonTitle:@"Ok" otherButtonTitles:nil];
    [alert setAlertViewStyle:UIAlertViewStyleDefault];
    [alert show];
    [connectedLabel setText:@"C"];
...
{
    connected = FALSE;
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Board Disconnected" message:@"The controller board has been disconnected" delegate:self cancelButtonTitle:@"Ok" otherButtonTitles:nil];
    [alert setAlertViewStyle:UIAlertViewStyleDefault];
    [connectedLabel setText:@"D"];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    if(buttonIndex == 1)
        return;
    if([alertView.title isEqualToString:@"No Files Saved"])
        return;
    if([alertView.title isEqualToString:@"Board Connected"])
        return;
    if([alertView.title isEqualToString:@"Board Disconnected"])
        return;
