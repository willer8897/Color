...
- (IBAction)dismiss
{
    NSArray *files = [appDelegate getFileNames];
    NSString *text;
    BOOL overwrite = FALSE;
    for(int i = 0; i < [files count] -1; i++)
    {
        NSString *fileName = [files objectAtIndex:i];
        fileName = [fileName stringByReplacingOccurrencesOfString:@".dat" withString:@""];
        NSLog(@"File name is: %@", fileName);
        if([appDelegate.fileName isEqualToString:fileName])
        {
            overwrite = TRUE;
            text = @"Overwrite the file: ";
            text = [text stringByAppendingString:appDelegate.fileName];
            text = [text stringByAppendingString:@" ?"];
            UIAlertView *saveAlert = [[UIAlertView alloc] initWithTitle:@"Save Targets" message:text delegate:self cancelButtonTitle:@"Overwrite" otherButtonTitles:@"Don't Overwrite" , nil];
